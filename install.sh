...
# Versions
RASA_X_VERSION=${RASA_X_VERSION:-0.34.0}
RASA_VERSION=${RASA_VERSION:-2.2.2}
...
provide_login_credentials() {
  ...
  echo -e "The deployment is ready đŸ‰. "
...
then
  upgrade_rasa_x
  wait_till_deployment_finished
  echo -e "Upgrading was successful đŸ‰. "
else
  generate_not_yet_specified_passwords
  install_rasa_x_chart
  wait_till_deployment_finished
